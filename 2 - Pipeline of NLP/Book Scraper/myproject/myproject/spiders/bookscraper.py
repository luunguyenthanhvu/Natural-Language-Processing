...
from myproject.items import BookscraperItem
import os
...
class BookScraper(CrawlSpider):
  ...
  rules = (
    Rule(LinkExtractor(restrict_css=".nav-list > li > ul > li > a"),
         follow=True),
  ...
  def parse_book(self, response):
    ...
    book_item["upc"] = response.css(
      ".table.table-striped > tr:nth-child(1) > td::text").get()
    book_item["url"] = response.url
    self.Save_to_file(book_item["image_url"], book_item["title"],
                      book_item["price"], book_item["upc"], book_item["url"])
    return book_item
